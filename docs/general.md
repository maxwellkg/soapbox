...
The application renders markdown through `ActionText::Markdown`, using a custom `MarkdownRenderer` that enables Rouge-backed syntax tokenization for fenced code blocks. That means authors write ordinary fenced markdown such as ```` ```ruby ```` and the renderer emits highlighted HTML with the usual Rouge token classes.

Styling is intentionally separate from rendering. The shared `markdown.css` stylesheet owns both the heading permalink presentation emitted by `MarkdownRenderer` and the Rouge token colors for fenced code blocks. The same stylesheet is also included in HTML emails so `premailer-rails` can inline those token colors for delivery. This keeps one formatting path for both channels without relying on browser-side highlighting or a mailer-only rendering fork. Generic typography and code-block structure remain in `terminal_theme.css`.
