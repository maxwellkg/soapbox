...
Styling is intentionally separate from rendering. The shared `syntax.css` stylesheet colors Rouge output on the web, and the same stylesheet is also included in HTML emails so `premailer-rails` can inline those token colors for delivery. This keeps one formatting path for both channels without relying on browser-side highlighting or a mailer-only rendering fork.
